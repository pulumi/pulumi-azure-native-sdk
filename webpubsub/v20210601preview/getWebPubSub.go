...
func LookupWebPubSubOutput(ctx *pulumi.Context, args LookupWebPubSubOutputArgs, opts ...pulumi.InvokeOption) LookupWebPubSubResultOutput {
	return pulumi.ToOutputWithContext(context.Background(), args).
		ApplyT(func(v interface{}) (LookupWebPubSubResultOutput, error) {
			...
			opts = utilities.PkgInvokeDefaultOpts(opts)
			var rv LookupWebPubSubResult
			secret, err := ctx.InvokePackageRaw("azure-native:webpubsub/v20210601preview:getWebPubSub", args, &rv, "", opts...)
			if err != nil {
				return LookupWebPubSubResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(LookupWebPubSubResultOutput)
			if secret {
				return pulumi.ToSecret(output).(LookupWebPubSubResultOutput), nil
			}
			return output, nil
...
// Diagnostic configuration of a Microsoft.SignalRService resource. Used together with Azure monitor DiagnosticSettings.
func (o LookupWebPubSubResultOutput) DiagnosticConfiguration() DiagnosticConfigurationResponsePtrOutput {
	return o.ApplyT(func(v LookupWebPubSubResult) *DiagnosticConfigurationResponse { return v.DiagnosticConfiguration }).(DiagnosticConfigurationResponsePtrOutput)
}
